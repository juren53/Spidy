...
class Browser(QMainWindow):
    ...
    def view_statistics(self):
        """Show statistics for the current webpage"""
        if not self.current_view():
            QMessageBox.warning(self, "No Page Loaded", "No webpage is currently loaded.")
            return

        def on_stats_ready(stats):
            """Callback for when statistics are ready"""
            dialog = self.create_statistics_dialog(stats)
            dialog.exec_()

        # Collect statistics and show them in the dialog
    ...
    def create_statistics_dialog(self, stats):
        ...
        # If there's an error, display it and return early
        if 'error' in stats:
            row += 1
            layout.addWidget(QLabel("<b>Error</b>"), row, 0, 1, 2)
            row += 1
            error_label = QLabel(stats.get('error', 'Unknown error'))
    ...
    def collect_page_statistics(self, callback):
        ...
        Args:
            callback: Function to call with the statistics once collected
        """
        # Safety check - make sure we have a valid view
        if not self.current_view():
            # No page loaded, return empty stats
            logging.warning("Attempted to collect page statistics with no active view")
            callback({
        ...
        try:
            # Log that we're about to execute JavaScript
            logging.debug("Executing JavaScript to collect page statistics")
            
            # First do another safety check to ensure we still have a valid view
            if not self.current_view() or not self.current_view().page():
                logging.error("View or page became invalid before JavaScript execution")
                callback({
                    'title': 'Error',
                    'url': 'Unknown',
                    'error': "Browser view or page became invalid"
                })
                return
                
            # Execute JavaScript asynchronously to collect statistics
            # Use the wrap_callback method to handle the result safely
            ...
            # Provide meaningful error information to the callback
            callback({
                'title': 'Error',
    ...
    def wrap_callback(self, callback):
        """
        Wrap a JavaScript callback with error handling and data validation
        
        This method creates a handler function that safely processes 
        data returned from JavaScript execution and passes it to the callback.
        The current view's state is captured at setup time to avoid inconsistencies
        when the callback fires later.
        
        Args:
            callback: The function to call with the processed JavaScript result
            
        Returns:
            A function that handles JavaScript results
        """
        # First check if we have a valid current view
        if not self.current_view():
            # We can't proceed without a valid view
            def error_handler(data=None):
                error_data = {
                    ...
                    'domain': 'None',
                    'protocol': 'None',
                    'error': 'No active page to collect statistics from'
                }
                # Call the original callback with our error data
                callback(error_data)
            
            return error_handler
        
        # Capture the current view's state at setup time to avoid issues if page changes
        current_view = self.current_view()
        current_url = current_view.url().toString() if current_view else 'Unknown'
        current_domain = current_view.url().host() if current_view else 'Unknown'
        current_protocol = current_view.url().scheme() if current_view else 'Unknown'
        current_title = current_view.title() if current_view else 'Unknown'
        
        # Log setup information
        logging.debug(f"Setting up JS callback for URL: {current_url}")
        
        # Prepare fallback data structure for error cases
        fallback_data = {
            'title': current_title or 'Error',
            'url': current_url,
            'domain': current_domain,
            'protocol': current_protocol,
            'error': 'Failed to retrieve page statistics'
        }
        
        # Create a handler function to process the JavaScript result
        def handle_js_data(data):
            try:
                # Debug info about the received data
                logging.debug(f"JS result type: {type(data)}")
                
                # Summarize data for logging (avoid huge outputs)
                if isinstance(data, dict):
                    data_summary = f"Dictionary with {len(data)} keys"
                    if len(data) < 10:  # Only show all keys if there aren't too many
                        data_summary += f": {list(data.keys())}"
                else:
                    # For non-dictionary results, show truncated representation
                    data_str = str(data)
                    data_summary = f"{data_str[:100]}{'...' if len(data_str) > 100 else ''}"
                
                logging.debug(f"JS result summary: {data_summary}")
                
                # Check if we got valid data
                if data is None:
                    logging.warning("JavaScript returned None")
                    fallback_data['error'] = 'JavaScript execution returned no data'
                    callback(fallback_data)
                    return
                
                # Check if data contains an error from JavaScript
                if isinstance(data, dict) and 'error' in data:
                    logging.warning(f"JavaScript error: {data['error']}")
                    fallback_data['js_error'] = data['error']
                    fallback_data['js_stack'] = data.get('stack', '')
                    callback(fallback_data)
                    return
                
                # Process the data based on its type
                if isinstance(data, dict):
                    # We got a valid dictionary - use it directly
                    logging.debug(f"Processing valid dictionary from JS with {len(data)} keys")
                    
                    # Ensure required fields exist (defensive programming)
                    for key in ['title', 'url', 'domain', 'protocol']:
                        if key not in data:
                    ...
                    logging.warning(f"Unexpected data format from JavaScript: {type(data)}")
                    fallback_data['js_response'] = str(data)[:500]  # Truncate very long responses
                    fallback_data['js_response_type'] = str(type(data))
                    callback(fallback_data)
            except Exception as e:
                # Catch any exceptions in the callback handling
                logging.error(f"Error in JavaScript callback handler: {str(e)}")
                # Print full traceback for debugging
                traceback.print_exc()
                
                # Prepare error data with captured values
                error_data = {
                    'title': current_title or 'Error',
                    'url': current_url,
                    'domain': current_domain,
